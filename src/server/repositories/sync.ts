...
export const syncRepository: SyncRepository = {
	async pushChanges(
		...
		// Entities are processed in foreign-key dependency order: note types →
		// field types → decks → notes → field values → cards → review logs. A
		// client that creates a note offline pushes the note and its cards in
		// the same request, so inserting cards before notes would violate
		// cards.note_id. Keep this order when adding entities.
